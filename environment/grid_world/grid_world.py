...
class GridWorld(Environment):
    def __init__(self, map_size: int, max_steps: int, render_configuration: Dict = None):
        super().__init__()
        ...
        self.observation_space: spaces.Dict = spaces.Dict(
            {
                'agent_position': spaces.Box(0, self._map_size - 1, shape=(2,), dtype=int),
                'target_position': spaces.Box(0, self._map_size - 1, shape=(2,), dtype=int),
            }
        )
    ...
    def _set_initialisation(self):
        ...
        self._agent_position = np.array([random.randint(0, self._map_size - 1), random.randint(0, self._map_size - 1)])
        self._target_position = np.array([random.randint(0, self._map_size - 1), random.randint(0, self._map_size - 1)])
    ...
    def step(self, action):
        ...
        self._agent_position = np.clip(self._agent_position + direction, 0, self._map_size - 1)
